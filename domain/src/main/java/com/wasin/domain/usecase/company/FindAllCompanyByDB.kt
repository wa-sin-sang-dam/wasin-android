...
class FindAllCompanyByDB @Inject constructor(
    private val companyRepository: CompanyRepository
) {
    operator fun invoke(): Flow<Resource<FindAllCompanyByDBResponse>> = flow {
        try {
            emit(Resource.Loading())
            val result = companyRepository.findAllCompanyByDB()
            val success = result.response ?: FindAllCompanyByDBResponse(emptyList())
